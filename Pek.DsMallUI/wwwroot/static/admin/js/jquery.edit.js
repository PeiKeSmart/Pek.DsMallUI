$(document).ready(function () {
    ...
    $('span[ds_type="inline_edit"]').click(function () {
        ...
        var ajax_ctype = $(this).attr('ajax_ctype');
        console.log('触发',this);
        $('<input type="text">')
            .attr({ value: s_value })
            .insertAfter($(this))
            ...
            .keyup(function (event) {
                if (event.keyCode == 13) {
                    ...
                    //value 修改内容
                    //type 国家0 省份1
                    $.get(ADMINSITEURL+'/'+action+'/ajax.html', { branch: ajax_branch, id: s_id, column: s_name, value: $(this).prop('value'),ctype:ajax_ctype   }, function (data) {
                        console.log(data);
                        if (data === 'false') {
                            alert('名称已经存在，请您换一个');
            ...
            .blur(function () {
                ...
                $(this).prev('span').show().text($(this).prop('value'));
                // 这个是国家省份的排序
                $.get(ADMINSITEURL+'/'+action+'/ajax.html', { branch: ajax_branch, id: s_id, column: s_name, value: $(this).prop('value'), type: $(this).prev().attr("typeof") , ctype:ajax_ctype   }, function (data) {
                    if (data === 'false') {
                        alert('名称已经存在，请您换一个');
                        $('span[fieldname="' + s_name + '"][fieldid="' + s_id + '"]').text(s_value);
                        return;
