...
/** checked的选中事件 -Xavier*/
function checkBoxFn(_this,id) {
    var obj = document.getElementById(id)
    if (_this.checked) {
        obj.value += _this.value + ','
    }else{ //删除
        obj.value = obj.value.replace(_this.value + ',', '')
    }                                           
}
/** 初始化select的内容 -Xavier*/
function initSelection(_self) {
    var url = HOMESITEURL + '/biz/GoodsSpec/GetGoodsClass';
        $.getJSON(url, {'pid': 0}, function (data) {
            if (data&&data.length>0) {
                for (i = 0; i < data.length; i++) {
                    $('#'+_self).append("<option value='" + data[i].Id + "'>" + data[i].Name + "</option>");
                }  
                $('#'+_self).change(gcategoryChange) 
            }
            else {
...
function gcategoryChange() {
    ...
    // console.log('选择事件-->',val);
    $('#class_id').val(val) //这个是不管什么都要
    // 找这个this中哪个option被选中了

    const isDefault = $(this).find('option:selected').attr('data-isDefault');
    console.log(isDefault);
    if (isDefault || !val) { //
        $(this).nextAll("select").remove();   //0（不选） 或者 空
        return 
    }else{
        $(this).nextAll("select").remove();  //继续
    }
    if (val != '') {
        $('#subClassificationId').val(val)
    } 
    
    const index = $(this).attr('data-index');
    // console.log('下标',index);
    // 还没有 需新增select && $(this).next("select").length == 0
    if (index<2 ) {
        var _self = this;
        var url = HOMESITEURL + '/biz/GoodsSpec/GetGoodsClass';
        // ajax请求下级分类
        $.getJSON(url, {'pid': this.value}, function (data) {
            if (data&&data.length>0) {
                $("<select name='rubbish' style='width:80px !important;' data-index='" + Number($('#gcategory').find('select').length) + "'><option value='"+val+"' data-isDefault='true'>" + "请选择" + "</option></select>").change(gcategoryChange).insertAfter(_self);
                for (i = 0; i < data.length; i++) {
