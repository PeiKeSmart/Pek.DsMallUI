...
function ds_ajaxpost(formid, type, url, waittime) {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            //layer.msg(res.msg, { time: waittime }, function () {
            //    //if (res.code == 10000) {
...
(function ($) {
    $.fn.ds_show_dialog = function (options) {
        ...
        var init_dialog = function (title) {
            ...
            that.draggable && that.draggable({ handle: ".dialog_head" });
        };
...
(function ($) {
    $.fn.ds_region = function (options) {
        ...
        settings.last_text = "";
        settings.hasregion = true;
        this.each(function () {
            var $inputArea = $(this);
            if ($inputArea.attr('data-name') === "") {
                initArea($inputArea);

                $("#" + options.Country).change(function () {
                    Cid = $(this).val();

                    if (settings.hasregion) {
                        settings.hasregion = false;
                    }
                    else {
                        ds_a = null;
                ...
                var htmlText = $inputArea.attr('data-name')
                console.log('这是什么-->',$inputArea);
                var $region_span = $('<span id="_area_span" class="' + settings.region_span_class + '">' +formaterStr(htmlText) + "</span>");
                var $region_btn = $('<input type="button" class="input-btn" style="margin-left:10px;" ' + settings.btn_style_html + ' value="编辑" />');
                ...
                $("#" + options.Country).change(function () {
                    ...
                    if (settings.hasregion) {
                        ...
                        initArea($inputArea);
                        //console.log($inputArea.attr('data-name'));
                    }
                });
            ...
            if ($inputArea.attr('data-name') && /^\d+$/.test($inputArea.attr('data-name'))) {
                $.getJSON(HOMESITEURL + "/Index/json_area_show?area_id=" + $inputArea.attr('data-name') + "&callback=?", function (data) {
                    $("#_area_span").html(data.text == null ? "无" : data.text)
                })
            }
        });
        /** 格式化省市名称 */
        function formaterStr(str) {
        ...
        function initArea($inputArea) {
            settings.$area = $("<select id='proviceSelect' style='padding:2px 10px;width:302px;'></select>");
        ...
        function loadArea($area, area_id) {
            // console.log('不执行？',$area,ds_a);
            if ($area && ds_a != null && ds_a.length > 0) {
                var areas = [];
                areas = ds_a;
                let html = ''; //有鬼
            ...
            $area.on("change", function () {
                ...
                $region.parent().find("select").each(function () {
                    if ($(this).find("option:selected").val() != "") {
                        region_value += $(this).find("option:selected").val() + " ";
                        area_ids.push($(this).find("option:selected").val())
                    }
                });
                const selectValue = $(this).val()
                $('#downtown').val('');
                // console.log('省份选择事件',selectValue,ParentCode);
                if (selectValue) {
                    console.log(selectValue);
                    areas.find(item => {
                        if (item.Id == selectValue) {
                            console.log('执行');
                            reloadSelectFn(item.AreaCode)
                            return
                        }
                    })
                }
        ...
        function loadAreaArray(callback) {
            ...
            Cid = $region.val();
            // http://localhost:9090/ 
            $.get(HOMESITEURL+ "/biz/Regions/GetbyCId?src=" + settings.src + "&CId=" + Cid + "&callback=?",function (data) {
                ds_a = data.Data;
...
// 重载select
var reloadSelectFn = function (id) {
     // 接受一个请求体带参数的对象
        const downtownSelectDom = $('#downtownSelect')
        if (downtownSelectDom.length === 0) return;
        // 删掉之前的select
        downtownSelectDom.remove();
        var settings = {
            $area: $("<select id='downtownSelect' style='padding:2px 10px;width:302px;'></select>"),
            resData:[],
            id,
        };
        load()
        loadAreaArray()
        function loadAreaArray() {
            // http://localhost:9090/ 
            $.get(HOMESITEURL+ "/biz/Regions/GetbyPId?src=" + settings.src + "&Code=" + settings.id + "&callback=?",function (data) {
        ...
        function loadArea(data) {
            ...
            $area.on("change", function () { 
               $('#downtown').val(this.value)
            //    console.log('重载后选择事件',this, $('#downtown').val());
            })
        }
        function load() {
            $("#downtown").after(settings.$area);
        }
};
    ...
    $.fn.downtown = function (options) {
        ...
        this.each(function () {
            ...
            if ($inputArea.attr('data-name') === "") {
                ...
                var htmlText = $inputArea.attr('data-name')
                var $region_span = $('<span id="_downtown_span" class="' + settings.region_span_class + '">' +formaterStr(htmlText) + "</span>");
                var $region_btn = $('<input type="button" class="input-btn" style="margin-left:10px;" ' + settings.btn_style_html + ' value="编辑" />');
                $inputArea.after($region_span);
                $region_span.after($region_btn);
                $region_btn.on("click", function () {
            ...
            if ($inputArea.attr('data-name') && /^\d+$/.test($inputArea.attr('data-name'))) {
                $.getJSON(HOMESITEURL + "/Index/json_area_show?area_id=" + $inputArea.attr('data-name') + "&callback=?", function (data) {
                    $("#_downtown_span").html(data.text == null ? "无" : data.text)
                })
            }
        });
        /** 格式化省市名称 */
        ...
        function initArea($inputArea) {
            settings.$area = $("<select id='downtownSelect' style='padding:2px 10px;width:302px;'></select>");
            $inputArea.before(settings.$area);
            loadAreaArray(function () {
                // console.log('为何',this);
                loadArea(settings.$area, settings.area_id)
        ...
        function loadArea($area, area_id) {
            // console.log('不执行？',$area,area_id,ds_a);
            if ($area && ds_a != null && ds_a.length > 0) {
                var areas = [];
                areas = ds_a;
                if (settings.tip_type && settings.last_text != "") {
            ...
            $area.on("change", function () {
                ...
                settings.last_text = $region.prevAll("select").find("option:selected").last().text();
                var area_id = settings.area_id = $(this).val();
                if ($('#_downtown2').length > 0) $("#_downtown2").val(settings.area_id_2);
                // ok
                if (settings.show_deep > 0 && $region.prevAll("select").size() == settings.show_deep) {
                    settings.islast = true;
                    if (typeof settings.last_click == 'function') {
        ...
        function loadAreaArray(callback) {
            ...
            // http://localhost:9090/ 
            // console.log('这里',ParentCode);
            $.get(HOMESITEURL+ "/biz/Regions/GetbyPId?src=" + settings.src + "&Code=" + $('#downtown').attr('data-parentId') + "&callback=?",function (data) {
                ds_a = data.Data;
                console.log('返回结果',data);
                callback()
            })
