...
$(function(){
    ...
    //     paramName: 'aPic',  // 指定文件参数名
    //     add: function (e, data) {
    //         // 添加文件大小检查
    //         var fileSize = data.files[0].size / 1024;  // 转换为 KB
    //         if (fileSize > 1000) {
    //             alert('图片大小不能超过1MB！');
    //             return false;
    //         }
    ...
    //         } else {
    //             // 设置图片ID和路径
    //             $('input[dstype="goods_image"]').val(result.data.Name);
    //             $('img[dstype="goods_image"]').attr('src', result.msg);
    //         }
    //     },
    ...
    $('#add_albums').fileupload({
        dataType: 'json',
        //url: HOMESITEURL+'/Sellergoodsadd/image_upload.html',
        url: CreateImgs,
        formData: { name: 'add_albums' },
        add: function (e, data) {
            $('i[dstype="add_album_is"]').html("&#xe717;").addClass('rotate').attr('data_types', parseInt($('i[dstype="add_album_i"]').attr('data_type')) + 1);
            data.submit();
        },
        done: function (e, data) {
            var _counter = parseInt($('i[dstype="add_album_is"]').attr('data_types'));
            _counter -= 1;
            if (_counter == 0) {
                $('i[dstype="add_album_is"]').removeClass('rotate').html("&#xe733;");
                $('a[dstype="show_descs"]').click();
            }
            $('i[dstype="add_album_is"]').attr('data_type', _counter);
        }
    });
    ...
    $('input[name="goodsState"]').click(function(){
        if($(this).attr('dstype') == 'auto'){
            ...
            $('#starttime_i').prop('disabled','disabled').css('background','#E7E7E7 none');
             // 清空时间值
             $('input[name="goodsShelftime"]').val('0');
        }
    });
    ...
    function updateGoodsShelftime() {
        var date = $('#starttime').val();
         var hour = $('#starttime_H').val();
         var minute = $('#starttime_i').val();
         
         // 组合完整的时间字符串
         var fullDateTime = date + ' ' + hour + ':' + minute + ':00';
         var timestamp = new Date(fullDateTime).getTime();
         $('input[name="goodsShelftime"]').val(timestamp);
     }
     $(function(){
         // 为时间相关控件添加change事件
         $('#starttime, #starttime_H, #starttime_i').on('change', function() {
             updateGoodsShelftime();
         });
     });
    ...
    /* 预约预售控制 */
    // 预约--出售时间
    $('#appointSatedate').datepicker({dateFormat: 'yy-mm-dd', minDate: new Date()});
    $('[name="isAppoint"]').change(function(){
    ...
    $('a[dstype="mb_add_img"]').click(function(){
        $('a[dstype="meat_cancel"]').click();
        $('div[dstype="mea_img"]').show().load(Selleralbum);
    });
    // 关闭 图片选择
...
function computeWeight(){
    // 库存
    var _weight = $('input[data_type="goods_weight"]').eq(0).val();
    $('input[name="goods_weight"]').val(parseInt(_weight));
}
// 计算价格
...
function insert_editor1(file_path) {
    ue1.execCommand('insertimage', { src: file_path });
}
