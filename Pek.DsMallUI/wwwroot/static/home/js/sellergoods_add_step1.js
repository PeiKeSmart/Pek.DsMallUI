...
function selClass($this){
    ...
    $('#dataLoading').show();
    //data_str.deep = data_str.deep + 1;
    var deep = parseInt(data_str.deep) + 1;
    $.getJSON(Querytheson, { gc_id: data_str.gcid, deep: deep }, function (data) {
        //if (data != '') {
        if (data.length != 0) {
            ...
            $.each(data, function(i, n){
                $('#class_div_' + deep).children('ul').append('<li data-param="{gcid:'
                        ...
                        + n.gc_id +',deep:'+ deep +'}"><a class="" href="javascript:void(0)"><i class="iconfont"></i>'
                        + n.gc_name + '</a></li>')
            ...
            //$('#class_div_' + data_str.deep + 1).parents('.sort_list:first').nextAll('div').children('div').addClass('blank').children('ul').html('');
...
$(function(){
    ...
    //    $.get(HOMESITEURL, { stapleid: data_str.stapleid }, function (data) {
    //        if (data.done) {
