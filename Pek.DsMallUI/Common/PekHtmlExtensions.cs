...
/// 表示 HTML 扩展
/// </summary>
public static class PekHtmlExtensions
{
    ...
    private static IHtmlContent? ForTreeEditor(IHtmlHelper Html, FieldItem? field, IEntityTree? entity)
    {
        ...
        if (entity?.GetType().GetValue("Setting") is not IEntityTreeSetting set || set.Parent != field?.Name) return null;
    ...
    private static IHtmlContent? ForMap(IHtmlHelper Html, FieldItem? field, IEntity? entity)
    {
        ...
        if (map.Provider == null || fact != null && fact.Session.Count > DHSetting.Current.MaxDropDownList)
        {
        ...
        // 表单页的映射下拉，开头增加无效值选项
        if (fact != null && map?.Provider.Key.IsNullOrEmpty() == false)
        {
    ...
    public static IHtmlContent? ForObject(this IHtmlHelper Html, String? name, Object? value)
    {
        ...
        return new HtmlString(sb.Return(true));
    }
    ...
    {
        var dic = NewLife.EnumHelper.GetDescriptions(value?.GetType()!);
    ...
    {
        var dic = NewLife.EnumHelper.GetDescriptions(typeof(T));
    ...
    /// <returns></returns>
    public static IHtmlContent ForDropDownList(this IHtmlHelper Html, String? name, IEnumerable items, Object? selectedValue = null, String? optionLabel = null, Boolean autoPostback = false, Object? htmlAttributes = null)
    {
        SelectList data;
        ...
        if (DHSetting.Current.BootstrapSelect)
        ...
        if (htmlAttributes is IDictionary<String, String> dic)
        {
            foreach (var item in dic)
            {
    ...
    public static IHtmlContent ForDropDownList<T>(this IHtmlHelper Html, String name, IList<T> list, Object? selectedValue = null, String? optionLabel = null, Boolean autoPostback = false) where T : IEntity
    {
        ...
        var uk = (fact?.Unique) ?? throw new InvalidDataException($"实体类[{type.FullName}]缺少唯一主键，无法使用下拉！");
